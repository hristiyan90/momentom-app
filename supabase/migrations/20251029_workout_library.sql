...
-- Sprint 1.5 GAP-3: Workout Library Seeding
-- =====================================================
-- Creates workout_library table to store workout templates
-- that can be used by the plan generation engine
...
-- Indexes
CREATE INDEX idx_workout_library_sport ON public.workout_library(sport);
CREATE INDEX idx_workout_library_phase ON public.workout_library(phase);
CREATE INDEX idx_workout_library_zone ON public.workout_library(primary_zone);
CREATE INDEX idx_workout_library_sport_phase ON public.workout_library(sport, phase);
CREATE INDEX idx_workout_library_focus_tags ON public.workout_library USING GIN(focus_tags);
...
-- RLS Policies
-- Note: Workout library is read-only for all authenticated users
-- Only administrators should be able to modify the library
...
AS $$
  ...
  FROM public.workout_library
  WHERE sport = p_sport
    AND focus_tags && p_required_tags  -- Array overlap operator
  ORDER BY
    -- Prioritize workouts with more matching tags
...
-- To rollback, run:
-- DROP FUNCTION IF EXISTS public.search_workouts_by_tags(TEXT, TEXT[]);
-- DROP FUNCTION IF EXISTS public.get_workouts_by_zone(TEXT, TEXT, INTEGER);
-- DROP FUNCTION IF EXISTS public.get_workouts_by_sport_phase(TEXT, TEXT);
